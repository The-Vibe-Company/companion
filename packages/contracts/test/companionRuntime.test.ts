...
import type { CompanionTurn } from "../src/companionRuntime";
...
describe("Companion Runtime v3 public contracts", () => {
  ...
  it("exposes replying only for positively acknowledged Turn admission", () => {
    expect(companionActiveTurnSchema.parse(turn()).replying).toBe(true);
    expect(() => companionTurnSchema.parse(turn({
      admission_state: "pending", admitted_at: null,
    }))).toThrow();
    expect(() => companionTurnSchema.parse(turn({
      status: "needs_input",
    }))).toThrow();
  ...
  it("projects Runtime v3 admission directly on the Turn", () => {
    const admitted = turn({
      status: "admitted",
      admission_state: "accepted",
      admitted_at: createdAt,
