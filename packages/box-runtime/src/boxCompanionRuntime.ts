...
function companionCapabilityInstructions(includeHub: boolean): string {
  ...
  if (includeHub) {
    lines.push(
      ...
      "  `not connected` or `cached` is idle and available on demand; cache-only search or list does not connect it.",
      "  The adapter connects it automatically on the first live tool or resource call.",
