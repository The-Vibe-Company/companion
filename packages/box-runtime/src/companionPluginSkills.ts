...
const COMMON_TRIGGER_RULES = [
  ...
  "owner-approved: request it with `companion_request_trigger_change` on the companion-control MCP",
  "(name, prompt, provider, and — where supported — a target), never claim one is active before approval.",
  "",
  "Approval creates and registers supported provider webhooks end-to-end with the approving member's",
  "held credential. Never pass or invent a provider account id: one eligible account is reused",
  "silently, while an ambiguous account choice stays fail-closed. A `failed` registration status means the provider refused — read",
