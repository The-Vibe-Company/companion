...
Hosted runtime protocol 6 never replays a prompt whose dispatch outcome is ambiguous. It attempts
one exact bounded Pi stop before terminal settlement, preserves the original interruption, marks
that occurrence `auto_abandoned`, and releases its execution lane even if cleanup fails. There is no
Retry/Cancel gate or durable recovery operation; later work continues automatically. First-party
...
```sh
printf '%s' '{"action":"api","method":"POST","path":"/local-skills/companion/installed","body":{"version":"1.104.0","agent":"<your assistant name>"}}' \
  | node scripts/companion-agent-client.mjs
```
