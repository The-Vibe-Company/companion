...
process.env.__AGENTHANGAR_PACKAGE_ROOT = resolve(__dirname, "..");
...
Server commands:
  ...
  status      Show service status (or use 'agenthangar status' when server is running)
...
Management commands (requires running server):
  ...
  assistant   Manage the AgentHangar Assistant (status, launch, stop, config)

Options:
  --port <n>  Override the default port (default: 6060)
`);
}
...
switch (command) {
  ...
  case "status": {
    ...
    try {
      ...
      if (!result.installed) {
        ...
        console.log("Run: agenthangar install");
      } else if (result.running) {
        console.log(`AgentHangar is running (PID: ${result.pid})`);
        console.log(`  URL: http://localhost:${result.port}`);
      } else {
        console.log("AgentHangar is installed but not running.");
        console.log("Check logs at ~/.agenthangar/logs/");
      }
    }
    break;
  }
  ...
  case "logs": {
    ...
    const logFile = join(homedir(), ".agenthangar/logs/agenthangar.log");
    const errFile = join(homedir(), ".agenthangar/logs/agenthangar.error.log");
    const { existsSync } = await import("node:fs");
    if (!existsSync(logFile) && !existsSync(errFile)) {
      console.error("No log files found at ~/.agenthangar/logs/");
      console.error("The service may not have been started yet.");
      process.exit(1);
    }
    console.log("Tailing logs from ~/.agenthangar/logs/");
