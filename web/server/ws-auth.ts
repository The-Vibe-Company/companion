 ...
 * Accepts token in query param or agenthangar_token cookie (query takes precedence).
 */
export async function authenticateManagedWebSocket(req: Request): Promise<WsAuthResult> {
  const secret = process.env.AGENTHANGAR_AUTH_SECRET?.trim();
  if (!secret) {
  ...
  const cookieToken = getCookie(req.headers.get("cookie"), "agenthangar_token");
