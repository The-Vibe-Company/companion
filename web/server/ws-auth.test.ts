...
describe("authenticateManagedWebSocket", () => {
  const savedSecret = process.env.AGENTHANGAR_AUTH_SECRET;
  ...
  afterEach(() => {
    if (savedSecret === undefined) delete process.env.AGENTHANGAR_AUTH_SECRET;
    else process.env.AGENTHANGAR_AUTH_SECRET = savedSecret;
  });
