...
export function reportProtocolDrift(
  ...
  emitError?.(
    `${options.backend === "codex" ? "Codex" : "Claude"} protocol drift: unsupported ${options.direction} ${options.messageKind} "${options.messageName}". AgentHangar may need an update.`,
  );
}
