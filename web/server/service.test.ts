...
function plistPath(): string {
  return join(tempDir, "Library", "LaunchAgents", "ai.blocksec.agenthangar.plist");
}
...
function unitPath(): string {
  return join(tempDir, ".config", "systemd", "user", "agenthangar.service");
}

function logDir(): string {
  return join(tempDir, ".agenthangar", "logs");
}
...
describe("generatePlist", () => {
  it("generates valid XML with the correct label", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/agenthangar" });
    expect(plist).toContain('<?xml version="1.0"');
    expect(plist).toContain("<string>ai.blocksec.agenthangar</string>");
  });
  ...
  it("uses the provided binary path in ProgramArguments", () => {
    const plist = service.generatePlist({ binPath: "/opt/homebrew/bin/agenthangar" });
    expect(plist).toContain("<string>/opt/homebrew/bin/agenthangar</string>");
  ...
  it("uses the default production port when none specified", () => {
    ...
    expect(plist).toContain("<string>6060</string>");
  });

  it("uses a custom port when specified", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/agenthangar", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/agenthangar", path: customPath });
...
describe("generateSystemdUnit", () => {
  it("generates a valid systemd unit with correct sections", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/agenthangar" });
  ...
  it("includes the description", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/agenthangar" });
    expect(unit).toContain("Description=AgentHangar");
  });

  it("uses the provided binary path in ExecStart", () => {
    const unit = service.generateSystemdUnit({ binPath: "/home/user/.bun/bin/agenthangar" });
    expect(unit).toContain("ExecStart=/home/user/.bun/bin/agenthangar start --foreground");
  });

  it("uses the default production port when none specified", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/agenthangar" });
    expect(unit).toContain("Environment=PORT=6060");
  });

  it("uses a custom port when specified", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/agenthangar", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    const customPath = "/custom/bin:/other/bin";
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/agenthangar", path: customPath });
...
describe("install", () => {
  it("creates log directory and writes plist file", async () => {
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/usr/local/bin/agenthangar\n";
    ...
    expect(content).toContain("ai.blocksec.agenthangar");
  });
  ...
  it("migrates old launchd label before installing", async () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/agenthangar" })
      .replaceAll("ai.blocksec.agenthangar", "co.thevibecompany.companion");
...
describe("install (linux)", () => {
  ...
  it("creates log directory and writes systemd unit file", async () => {
    ...
    expect(content).toContain("ExecStart=/usr/local/bin/agenthangar start --foreground");
  });
...
describe("start (linux)", () => {
  ...
  it("calls systemctl start when installed", async () => {
    ...
    const startCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("start agenthangar.service"),
    );
    expect(startCall).toBeDefined();
  });
...
describe("stop (linux)", () => {
  ...
  it("calls systemctl stop when installed", async () => {
    ...
    const stopCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("stop agenthangar.service"),
    );
    expect(stopCall).toBeDefined();
  });
...
describe("restart (linux)", () => {
  ...
  it("calls systemctl restart when installed", async () => {
    ...
    const restartCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("restart agenthangar.service"),
    );
    expect(restartCall).toBeDefined();
  });
...
describe("status (linux)", () => {
  ...
  it("returns installed: true, running: true with PID", async () => {
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (typeof cmd === "string" && cmd.includes("show agenthangar.service")) {
...
describe("refreshServiceDefinition (macOS)", () => {
  it("rewrites plist with current binary path", async () => {
    ...
    expect(originalContent).toContain("/usr/local/bin/agenthangar");
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/new/path/agenthangar\n";
      return "";
    });
    ...
    expect(updatedContent).toContain("/new/path/agenthangar");
  });
