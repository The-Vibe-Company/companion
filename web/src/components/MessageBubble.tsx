...
import { SensitiveFileWriteApproval, isSensitiveFileRejection } from "./SensitiveFileWriteApproval.js";
...
function UserMessage({ message }: { message: ChatMessage }) {
  ...
  return (
    <div className="flex justify-end animate-[userSlideIn_0.3s_ease-out]">
      <div className="max-w-[85%] sm:max-w-[80%] px-3.5 sm:px-4 py-2.5 rounded-[16px] rounded-br-[6px] user-bubble-gradient text-cc-fg shadow-[0_1px_3px_rgba(0,0,0,0.04)] group relative">
        ...
        <button
          onClick={copyMessage}
          className={`absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity text-[10px] px-1.5 py-0.5 rounded ${
            copied
              ? "bg-cc-success/20 text-cc-success"
              : "bg-cc-fg/10 text-cc-fg/50 hover:bg-cc-fg/15 hover:text-cc-fg/70"
          } cursor-pointer`}
          title="Copy message"
        >
          {copied ? "✓" : "copy"}
...
function AssistantMessage({ message }: { message: ChatMessage }) {
  ...
  // Server-side detection of CLI's "sensitive file" Write rejection emits
  // sensitive_file_rejection events keyed by the rejected tool_use_id.
  // We render an Approve/Reject card right under the matching tool_use so
  // the user can recover without leaving the chat. Subscribe to just the
  // current session's map to avoid re-rendering on cross-session updates.
  const currentSessionId = useStore((s) => s.currentSessionId);
  const rejectionsForSession = useStore((s) =>
    currentSessionId ? s.sensitiveFileRejections.get(currentSessionId) : undefined,
  );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0 group relative">
          {isThinkingPhase ? (
            ...
            <MarkdownContent text={message.content} showCursor={!!message.isStreaming} showCopyButton={!message.isStreaming} />
          )}
        </div>
      </div>
    );
      ...
      <div className="flex-1 min-w-0 space-y-3">
        {grouped.map((group, i) => {
          ...
          if (group.items.length === 1) {
            ...
            const rejection = rejectionsForSession?.get(item.id);
            return (
              <div key={i} className="space-y-2">
                <ToolBlock name={item.name} input={item.input} toolUseId={item.id} />
                {rejection && (
                  <SensitiveFileWriteApproval content={rejection} toolUseId={item.id} />
                )}
              </div>
            );
          }
...
function MarkdownContent({ text, showCursor = false, showCopyButton = false }: { text: string; showCursor?: boolean; showCopyButton?: boolean }) {
  const [copied, setCopied] = useState(false);
  ...
  return (
    <div className={`markdown-body text-[14px] sm:text-[15px] text-cc-fg leading-relaxed overflow-hidden ${showCopyButton ? "group relative" : ""}`}>
      <Markdown
...
function CodeBlock({ lang, codeText }: { lang: string; codeText: string }) {
  ...
  return (
    <div className="my-2.5 rounded-xl overflow-hidden border border-cc-border shadow-[0_2px_8px_rgba(0,0,0,0.04)] group relative">
      {lang && (
        <div className="px-3 py-1.5 bg-cc-code-bg border-b border-cc-border flex items-center justify-between gap-2">
          ...
          <button
            ...
            className={`text-[10px] px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity ${
              copied
                ? "bg-cc-success/20 text-cc-success"
                : "bg-cc-muted/10 text-cc-muted/60 hover:bg-cc-muted/20 hover:text-cc-muted/80"
            } cursor-pointer`}
            title="Copy code"
          >
            {copied ? "✓" : "copy"}
...
function ContentBlockRenderer({
  ...
  if (block.type === "text") {
    return <MarkdownContent text={block.text} showCopyButton />;
  }

  if (block.type === "thinking") {
    // Opus 4.7 returns thinking blocks with `thinking: ""` and only a
    // signature (encrypted/extended-thinking format), so the visible
    // content is always empty. Hide the block in that case rather than
    // rendering "No thinking text captured." every turn — the user has
    // nothing useful to read. Older models still surface readable
    // thinking text, which renders as before.
    if (!block.thinking || block.thinking.trim().length === 0) return null;
    return <ThinkingBlock text={block.thinking} />;
  }

  // Anthropic's "redacted_thinking" block carries opaque encrypted data
  // with no human-readable text. Render nothing for the same reason.
  if ((block as { type: string }).type === "redacted_thinking") return null;
  ...
  if (block.type === "tool_result") {
    ...
    // CLI's internal sensitive-file guard auto-rejects Write with a fake
    // "permission" error that companion can't gate via PermissionBanner.
    // Render a real Approve/Reject UI instead of a misleading red error.
    if (isError && isSensitiveFileRejection(content)) {
      return <SensitiveFileWriteApproval content={content} toolUseId={block.tool_use_id} />;
    }
    if (toolName === "Bash") {
      return <BashResultBlock text={content} isError={isError} />;
    }
    return <TerminalOutputBlock text={content} isError={isError} />;
  }
...
function BashResultBlock({ text, isError }: { text: string; isError: boolean }) {
  ...
  const displayLines = showFull || !hasMore ? allLines : allLines.slice(-20);
  ...
  return (
    <div className="rounded-lg bg-cc-code-bg overflow-hidden">
      <div className={`text-[12px] font-mono-code whitespace-pre-wrap leading-relaxed ${
        isError ? "text-cc-error" : "text-cc-code-fg/60"
      }`}>
        {displayLines.map((line, idx) => {
          const actualIndex = showFull || !hasMore ? idx : allLines.length - displayLines.length + idx;
          const isCopied = copiedIndex === actualIndex;
          return (
            <div
              key={`${actualIndex}-${line}`}
              className="flex items-center gap-2 group px-3 py-1 hover:bg-cc-hover/30 transition-colors"
            >
              <div className="flex-1 min-w-0 break-words">{line || "\u00A0"}</div>
              <button
                onClick={() => copyLine(line, actualIndex)}
                className={`shrink-0 opacity-0 group-hover:opacity-100 transition-opacity text-[10px] px-1.5 py-0.5 rounded ${
                  isCopied
                    ? "bg-cc-success/20 text-cc-success"
                    : "bg-cc-muted/10 text-cc-muted/60 hover:bg-cc-muted/20 hover:text-cc-muted/80"
                } cursor-pointer`}
      ...
      {hasMore && (
        <div className="px-3 py-1.5 flex items-center justify-between border-t border-cc-border/30">
          <span className={`text-[10px] ${isError ? "text-cc-error/50" : "text-cc-muted/40"}`}>
            {showFull ? `${allLines.length} lines` : `last 20 of ${allLines.length}`}
          </span>
          <button
...
function TerminalOutputBlock({ text, isError }: { text: string; isError: boolean }) {
  const [copied, setCopied] = useState(false);
  ...
  return (
    <div className="rounded-lg bg-cc-code-bg overflow-hidden group relative">
