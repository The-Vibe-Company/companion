...
describe("companion skill package + row", () => {
  it("packs the bundled skill deterministically and reads its version", async () => {
    ...
    expect(pkg.version).toBe("1.106.0");
  ...
  it("builds a 'none' row with an actionable install prompt for a fresh caller", async () => {
    ...
    expect(changelog).toContain("outcome-unknown Runtime v3 Pi admission without replay");
    expect(changelog).toContain("exact Pi-only recycle on the same persistent Box");
    expect(changelog).toContain("only when complete continuity cannot be proved");
    ...
    const attachmentsChanges = manifest.metadata?.changelog
      ...
      ?.changes?.join("\n") ?? "";
    expect(attachmentsChanges).toContain("Companion chat attachments were added to the shared contracts");
    ...
    expect(runtimeBoundaryChanges).toContain("Agent Auth never grants hosted Companion chat");
    ...
    expect(memberStateChanges).toContain("Companion roster preferences and duplicate Box creation");
    ...
    expect(allowDenyChanges).toContain("Companion thread permission decisions and Pi extension UI responses");
    ...
    expect(toolchainChanges).toContain("Does not change Companion skill commands");
