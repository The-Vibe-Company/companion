...
|---|---|---|
| `--port <n>` | Override the server port | `6060` |
...
agenthangar install && agenthangar start && agenthangar status
...
|---|---|---|
| `AGENTHANGAR_AUTH_TOKEN` | Auth token (overrides `~/.agenthangar/auth.json`) | Auto-generated |
| `AGENTHANGAR_RECORD` | Set to `0` or `false` to disable protocol recording | `true` |
| `AGENTHANGAR_RECORDINGS_DIR` | Override recordings directory | `~/.agenthangar/recordings/` |
| `AGENTHANGAR_RECORDINGS_MAX_LINES` | Max total lines before recording rotation | `1,000,000` |
| `AGENTHANGAR_INIT_SCRIPT_TIMEOUT` | Init script timeout in seconds | `120` |
