...
description: Deploy AgentHangar on a private cloud VM with Tailscale access
---

# Deploy on a Cloud VM

AgentHangar runs locally by default, but you can deploy it on a remote server for always-on access, shared team usage, or dedicated development environments.

## Choosing a setup

| Scenario | Recommended approach |
|---|---|
| Solo developer, single machine | [Local install](/get-started/installation) with `bunx agenthangar` |
...
1. AgentHangar server runs on the host (port 6060 by default)
...
This guide walks you through deploying AgentHangar on a private GCP VM in the `europe-west9` (Paris) region. The VM has no public IP — you'll access it securely through Tailscale, which gives you an HTTPS endpoint without exposing anything to the internet.

By the end, you'll have a dedicated AgentHangar server running at `https://<hostname>.<tailnet>.ts.net/`.

### What you'll set up

| Component | Purpose |
|---|---|
| GCP Compute Engine VM | Runs AgentHangar (no public IP) |
...
export INSTANCE="agenthangar-vm"
...
export AGENTHANGAR_HOSTNAME="agenthangar"
...
The VM needs outbound internet access to download packages, pull Docker images, and install toolchains.
...
### 6. Install Bun and AgentHangar

SSH into the VM and install AgentHangar as a global package with a background service:
...
sudo -E tailscale up \
  --hostname=\"$AGENTHANGAR_HOSTNAME\" \
  --ssh
unset TS_AUTHKEY

# Serve AgentHangar over HTTPS
sudo tailscale serve --bg 6060
'"
```
...
Replace `<hostname>` with the value of `AGENTHANGAR_HOSTNAME` and `<tailnet>` with your Tailscale network name.
...
### AgentHangar commands

| Command | What it does |
|---|---|
| `agenthangar status` | Check if the service is running |
| `agenthangar logs` | Tail recent logs |
| `agenthangar restart` | Restart the server |
| `agenthangar stop` | Stop the server |
...
- AgentHangar generates an auth token automatically — see [Authentication](/get-started/installation#authentication) for details
