...
| A due Companion routine fires exactly once per scheduled instant, safely rejected starts retry under a fresh tombstone-safe Pi identity, and queued routine work is skipped on disable/delete or after ten minutes | Duplicate turn after worker retry, a retry permanently rejected by its prior cancellation tombstone, catch-up after flag-off, stuck work after a routine mutation, or a routine row blocking a user message | Core + worker + runtime + PostgreSQL + live Box canary | Drop uuidv5 stamping, post-cleanup invocation generation, the ten-minute cleanup, generation match, or the lane fence |
...
| Fresh staged MCP/control capabilities are loaded by an already-active Pi | Every selected MCP remains attached but calls return 401/502 through the Box gateway | Box command contract plus fully migrated PostgreSQL upgrade | Replace the credential-aware Pi recycle with an unconditional `systemctl start`, or expire claimed, admitted, ambiguous, terminal-ACK, or Pi-recovery work during the repair migration |
| An attached lazy MCP remains usable before its first active connection | Pi reports an idle plugin as unavailable or asks the member to reconnect it | Staged operating-brief unit test plus pinned adapter lazy-connect contract | Restore wording that equates only currently connected servers with available plugins |
| Failed Pi activation restages consumed credentials | A newly created Companion remains forever at `staged` while Pi restarts without credentials | Fully migrated PostgreSQL upgrade | Retain `staged` after an error-bearing activation deferral |
| Provider model pickers expose only models executable by the pinned Box runtime | A newly advertised pi.dev model creates a Companion whose Pi exits before readiness | Core provider-catalog contract | Return live model ids that are absent from the server-owned pinned allowlist |
