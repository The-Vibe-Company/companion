...
The default image (`agenthangar:latest`) is based on Ubuntu 24.04 and includes:
...
3. Select a **base image** from the dropdown (e.g., `agenthangar:latest`) or enter a custom image name
...
This will output a token you can set in your environment profile's **Variables** tab, or export in your shell before starting the AgentHangar.
</Warning>

AgentHangar automatically:
...
- Timeout: **120 seconds** (configurable via `AGENTHANGAR_INIT_SCRIPT_TIMEOUT` env var)
...
Profiles are stored as individual JSON files in `~/.agenthangar/envs/`, named by slug (e.g., `glm-v5.json`).
