...
AgentHangar lets you select a git branch when creating a session and optionally create an isolated git worktree so multiple agents can work on the same repo without interfering with each other.
...
1. AgentHangar creates a new working directory at `~/.agenthangar/worktrees/{repo-name}/{branch}/`
...
|---|---|---|
| First session on `feature/login` | `~/.agenthangar/worktrees/my-repo/feature--login/` | `feature/login` |
| Second session on `feature/login` | `~/.agenthangar/worktrees/my-repo/feature--login-wt-8374/` | `feature/login-wt-8374` |
| First session on `main` | `~/.agenthangar/worktrees/my-repo/main/` | `main` |
...
Worktrees are tracked in `~/.agenthangar/worktrees.json`. When you archive a session, its worktree can be cleaned up. Worktrees with uncommitted changes are flagged before deletion.
