...
1. The server spawns a CLI subprocess (Claude Code with `--print --input-format stream-json --output-format stream-json`, or Codex with `app-server`)
2. The server attaches its backend adapter to the child's stdin/stdout and exchanges NDJSON over the pipes
3. The server bridges those messages out to your browser over WebSocket
...
When an agent wants to perform a sensitive action — writing files, running commands, accessing the network — AgentHangar intercepts the request and presents it for your approval.
...
Sessions are saved to disk as JSON files in `$TMPDIR/agenthangar-sessions/`. Writes are debounced to minimize I/O during streaming. The saved state includes the session configuration, message history, task list, permission state, and CLI process PID.
...
This means you can restart AgentHangar without losing your work. The agent picks up where it left off.
