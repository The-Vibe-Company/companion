...
```bash
curl -X POST https://your-agenthangar:6060/api/agents/pr-reviewer/webhook/YOUR_SECRET \
...
```bash
curl http://localhost:6060/api/agents/pr-reviewer/executions \
...
```bash
# Export
curl http://localhost:6060/api/agents/pr-reviewer/export \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -o pr-reviewer.agent.json

# Import
curl -X POST http://localhost:6060/api/agents/import \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
...
Agents are stored as individual JSON files in `~/.agenthangar/agents/`. The agent ID is a slug derived from the name (e.g., "PR Reviewer" becomes `pr-reviewer`).
