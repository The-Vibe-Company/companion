...
AgentHangar integrates with [Linear](https://linear.app) to let you search issues, create new ones, link them to sessions, and automatically transition issue status as you work.
...
Mappings are stored in `~/.agenthangar/linear-projects.json`.

## Rate limiting

Linear has a 5,000 requests/hour rate limit. AgentHangar manages this with built-in caching:
